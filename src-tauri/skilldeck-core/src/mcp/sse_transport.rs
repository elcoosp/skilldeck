...
impl SseSessionInner {
    ...
    async fn send_request(
        ...
        Err(CoreError::McpJsonRpc {
            code: -32603,
            message: "No valid JSON-RPC response in SSE stream".into(),
        })
    }
    ...
    async fn initialize(&self) -> Result<McpCapabilities, CoreError> {
        ...
        let result: InitializeResult =
            serde_json::from_value(resp.result.ok_or_else(|| CoreError::McpJsonRpc {
                code: -32603,
                message: "No result in initialize".into(),
            })?)
...
mod tests {
    ...
    fn parse_url_ok() {
        ...
        assert_eq!(
            SseTransport::parse_url(&config).unwrap(),
            "http://localhost:3000"
        );
    }
