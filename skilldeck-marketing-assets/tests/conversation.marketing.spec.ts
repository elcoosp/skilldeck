...
test('Active Conversation Workflow', async ({ tauriPage }) => {
  ...
  await tauriPage.waitForSelector('[data-testid="app-shell"]', {
    timeout: 30000
  })
  ...
  await centerPanel.messageThread.waitForMessageWithText(
  ...
  await test
    .info()
    .attach('conversation', { body: screenshot, contentType: 'image/png' })

  // Video recording (handled automatically by the fixture on failure, but we can do it manually)
  await tauriPage.startRecording({
    path: test.info().outputPath('recording'),
    fps: 15
  })
