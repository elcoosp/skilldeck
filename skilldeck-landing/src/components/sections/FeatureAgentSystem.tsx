...
import { Network, Zap, ShieldCheck, Cpu, Bot, CheckCircle2, Clock, Loader2 } from 'lucide-react'
    ...
    <div className="lg:hidden rounded-2xl border border-border bg-card p-4 space-y-3">
      ...
      <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-2.5">
        ...
        <p className="text-[10px] text-muted-foreground mb-2">
          <span className="text-foreground font-medium">run_tests</span> -- <span className="font-mono text-[10px]">npm test</span>
        </p>
      ...
      <p className="text-[10px] text-muted-foreground flex items-center gap-1.5">
        ...
        {t`Merge:`} <span className="text-foreground font-medium">{t`summarize`}</span> -- 1/2 {t`done`}
      </p>
    </div>
  )
}
      ...
      <div className="flex h-[380px]">
        ...
        <div className="flex-1 flex flex-col min-w-0 p-4">
          ...
          <div className="flex justify-start mb-3">
            <div className="max-w-[90%]">
              <p className="text-xs text-muted-foreground mb-2">
                <span className="font-medium text-foreground">{t`Coordinator`}</span> -- {t`Spawning subagents for parallel review...`}
              </p>
              ...
              <div className="bg-card border border-amber-500/30 rounded-lg p-2.5">
                ...
                <p className="text-[10px] text-muted-foreground mb-2">
                  <span className="text-foreground font-medium">run_tests</span> -- {t`Execute test suite with`} <span className="font-mono text-xs">npm test</span>
                </p>
                <div className="flex gap-1.5">
                  <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 font-medium">{t`Approve`}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-muted-foreground border border-border font-medium">{t`Deny`}</span>
          ...
          <div className="mt-auto pt-2 border-t border-border/30">
            <p className="text-[10px] text-muted-foreground flex items-center gap-1.5">
              <Cpu className="w-3 h-3" />
              {t`Merge strategy:`} <span className="text-foreground font-medium">{t`summarize`}</span> -- {t`Waiting for 1/2 subagents`}
            </p>
          </div>
        </div>
      </div>
...
export function FeatureAgentSystem() {
  ...
  const bulletPoints = [
    ...
    { icon: ShieldCheck, text: t`Tool Approval Gate — review, edit, or block every external call before it executes` },
    { icon: Cpu, text: t`Parallel subagent spawning — merge results with concat, summarize, or vote strategies` },
    { icon: Network, text: t`Subagent cards with live status, progress tracking, and instant result access` },
  ]
