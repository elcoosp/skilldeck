...
import { Brain, Cpu, Plug, Sparkles } from 'lucide-react'
...
function ProviderCard({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 sm:px-6 sm:py-4 rounded-xl border border-border bg-card hover:border-primary/30 transition-all duration-300 cursor-default h-full overflow-hidden">
      <div
        className={`w-10 h-10 rounded-lg bg-card flex items-center justify-center shrink-0`}
      >
...
export function LogoCloud() {
  ...
  const providers = [
    ...
    },
    {
      name: 'Ollama',
      icon: Cpu,
      desc: 'Llama, Mistral, Gemma',
      iconColor: 'text-sky-500'
    },
    {
      name: 'MCP Protocol',
      icon: Plug,
      desc: t`Model Context Protocol`,
