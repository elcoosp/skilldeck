...
import { Shield, Zap, LockOpen, Bot } from 'lucide-react'
    ...
    <div className="sm:hidden rounded-2xl border border-border bg-card p-4 space-y-3">
      ...
      <div className="space-y-2">
        <div className="flex gap-2 items-start">
          <div className="w-5 h-5 rounded-full bg-blue-500/20 flex items-center justify-center text-[9px] text-primary shrink-0 mt-0.5">U</div>
          <div className="bg-background rounded-lg rounded-tl-none px-2.5 py-1.5 text-xs text-foreground">
            {t('Review the auth module')}
          </div>
        </div>
        <div className="flex gap-2 items-start">
          <div className="w-5 h-5 rounded-full bg-emerald-500/20 flex items-center justify-center text-[9px] text-emerald-500 shrink-0 mt-0.5">A</div>
          <div className="bg-background rounded-lg rounded-tl-none px-2.5 py-1.5 text-xs text-foreground">
    ...
    <div className="hidden sm:block w-full max-w-2xl mx-auto overflow-hidden">
      <div className="rounded-xl border border-border overflow-hidden bg-card shadow-2xl shadow-blue-500/5">
        <div className="flex items-center gap-2 px-4 py-3 bg-card border-b border-border">
          ...
          <span className="text-xs text-muted-foreground ml-2 truncate">{t('SkillDeck — Code Review')}</span>
        ...
        <div className="flex h-80">
          <div className="w-44 border-r border-border bg-background/50 p-3 space-y-2 shrink-0">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2 font-medium">{t('Agents')}</div>
            ...
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-4 mb-2 font-medium">{t('Status')}</div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex-1 p-4 space-y-3 overflow-hidden">
              <div className="flex gap-2 items-start">
                <div className="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center text-[10px] text-primary shrink-0 mt-0.5">U</div>
                <div className="bg-background rounded-lg px-3 py-2 text-sm text-foreground">
                  {t('Review the authentication module changes')}
                </div>
              </div>
              <div className="flex gap-2 items-start">
                <div className="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center text-[10px] text-emerald-500 shrink-0 mt-0.5">A</div>
                <div className="space-y-2 min-w-0">
                  <div className="bg-background rounded-lg px-3 py-2 text-sm text-foreground">
                    {t('I found 3 potential issues in the JWT implementation. Running deeper analysis...')}
                  </div>
                  <div className="flex items-center gap-2 rounded-lg border border-amber-400/20 bg-amber-400/5 px-3 py-1.5">
                    <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" />
                    <span className="text-xs text-amber-400">{t('Approval gate: awaiting human review')}</span>
                  </div>
                </div>
              </div>
            </div>
...
function TrustBadgeItem({ icon: Icon, label, desc }: { icon: React.ComponentType<{ className?: string }>; label: string; desc: string }) {
  return (
...
export function Hero() {
  ...
  const headline = headlineVariant === 'outcome'
    ? t('Run AI agents, workflows, and tools')
    : t('Stop stitching together')

  const headlineGradient = headlineVariant === 'outcome'
    ? t('from a single desktop app.')
    : t('AI tools that don\'t talk to each other.')

  const subheadline = headlineVariant === 'outcome'
    ? t('SkillDeck gives you multi-agent orchestration, a visual workflow editor, and a skill system with built-in linting — all running locally on macOS, Windows, or Linux. No account, no cloud, no vendor lock-in.')
    : t('SkillDeck puts multi-agent orchestration, visual workflows, and skill-based AI in one desktop app. Everything runs locally — your prompts, your code, your API keys never leave your machine.')
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center bg-background overflow-hidden">
      ...
      <div className="max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12 text-center">
        ...
        <p className="mt-6 text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">{subheadline}</p>
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button size="lg" className="bg-gradient-to-r from-blue-600 to-blue-400 hover:from-blue-500 hover:to-blue-300 text-white border-0 shadow-lg shadow-blue-500/25 text-base px-8 py-6">
            <Download className="w-5 h-5 mr-2" />{ctaText}
          </Button>
          <Button variant="outline" size="lg" className="border-border hover:bg-white/5 text-foreground text-base px-8 py-6" asChild>
            <a href="https://github.com/elcoosp/skilldeck" target="_blank" rel="noopener noreferrer">
              <Github className="w-5 h-5 mr-2" />{t('View on GitHub')}<ArrowRight className="w-4 h-4 ml-1" />
            </a>
          </Button>
        </div>
        <AppMockup />
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl mx-auto">
          {trustBadges.map((badge) => (
            <TrustBadgeItem key={badge.label} icon={badge.icon} label={badge.label} desc={badge.desc} />
          ))}
        </div>
      </div>
    </section>
