...
function useGitHubStats() {
  ...
  useEffect(() => {
    fetch('https://api.github.com/repos/elcoosp/skilldeck')
      .then((res) => res.json())
      .then((data) => { setStars(data.stargazers_count ?? 0); setForks(data.forks_count ?? 0) })
      .catch(() => {})
  }, [])
  return { stars, forks }
}
    ...
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight">
          {t`Free and open source —`} <span className="gradient-text">{t`forever`}</span>.
        </h2>
        ...
        <div className="mt-10 flex items-center justify-center gap-8 sm:gap-12">
          <div className="flex items-center gap-2">
            ...
            <span className="text-2xl font-bold text-foreground">{stars !== null ? formatNumber(stars) : '---'}</span>
            <span className="text-sm text-muted-foreground">{t`stars`}</span>
          </div>
          <div className="w-px h-8 bg-border" />
          <div className="flex items-center gap-2">
            <GitFork className="w-5 h-5 text-muted-foreground" />
            <span className="text-2xl font-bold text-foreground">{forks !== null ? formatNumber(forks) : '---'}</span>
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button size="lg" className="bg-gradient-to-r from-blue-600 to-blue-400 hover:from-blue-500 hover:to-blue-300 text-white border-0 shadow-xl shadow-blue-500/25 text-base px-8" asChild>
            <a href="https://github.com/elcoosp/skilldeck" target="_blank" rel="noopener noreferrer">
              <Github className="w-5 h-5 mr-2" />{t`Star on GitHub`}
            </a>
          </Button>
          <Button variant="outline" size="lg" className="border-border hover:bg-white/5 text-foreground text-base px-8" asChild>
            <a href="https://github.com/elcoosp/skilldeck/fork" target="_blank" rel="noopener noreferrer">
              <GitFork className="w-5 h-5 mr-2" />{t`Fork & Contribute`}
            </a>
          </Button>
        </div>
      </div>
