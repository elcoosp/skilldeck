...
function CodeLine({ line, lineNumber }: { line: string; lineNumber: number }) {
  ...
  else if (trimmed.startsWith('name:') || trimmed.startsWith('license:') || trimmed.startsWith('allowed_tools:')) className = 'text-primary'
  ...
  else if (trimmed.startsWith('1.') || trimmed.startsWith('2.') || trimmed.startsWith('3.') || trimmed.startsWith('4.')) className = 'text-foreground'
  else if (trimmed.startsWith('- ')) className = 'text-foreground'
  else if (trimmed.startsWith('|')) className = 'text-emerald-500/70'
  else if (trimmed.startsWith('Run') || trimmed.startsWith('For each') || trimmed.startsWith('Group') || trimmed.startsWith('Output')) className = 'text-foreground'

  return (
    <div className="flex hover:bg-white/[0.02] -mx-5 px-5">
      <span className="select-none text-muted-foreground/30 w-8 text-right mr-4 shrink-0">{lineNumber}</span>
