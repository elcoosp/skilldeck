        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <Input ref={inputRef} type="email" required placeholder="you@example.com" value={email} onChange={(e) => { setEmail(e.target.value); if (status === 'error') setStatus('idle') }} disabled={status === 'loading'} className="flex-1 h-12 px-4 bg-white/5 border-border/50 text-foreground placeholder:text-muted-foreground focus:border-blue-500/50 focus:ring-blue-500/20" />
              <Button type="submit" disabled={status === 'loading' || !email.trim()} size="lg" className="h-12 px-8 bg-gradient-to-r from-blue-600 to-blue-400 hover:from-blue-500 hover:to-blue-300 text-white border-0 shadow-lg shadow-blue-500/20 shrink-0">
                {status === 'loading' ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />{t`Joining...`}</>) : t`Join Waitlist`}
              </Button>
            </div>
