...
export function Footer() {
  ...
  const footerLinks: Record<string, { label: string; href: string }[]> = {
    ...
    [t`Community`]: [
      ...
      { label: t`Contributing`, href: 'https://github.com/elcoosp/skilldeck/blob/main/CONTRIBUTING.md' },
    ],
    [t`Legal`]: [
        ...
        <div className="py-12 lg:py-16">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            <div className="col-span-2 md:col-span-1">
              ...
              <a href="https://github.com/elcoosp/skilldeck" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                <Github className="w-4 h-4" />{t`Star us on GitHub`}
              </a>
            </div>
            {Object.entries(footerLinks).map(([category, links]) => (
              <div key={category}>
                ...
                <ul className="space-y-2">
                  {links.map((link) => (
                    <li key={link.label}>
                      <a href={link.href} className="text-sm text-muted-foreground hover:text-foreground transition-colors" {...(link.href.startsWith('http') ? { target: '_blank', rel: 'noopener noreferrer' } : {})}>{link.label}</a>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mt-12 pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">&copy; {new Date().getFullYear()} SkillDeck. {t`Open source under MIT License.`}</p>
          </div>
        </div>
      </div>
    </footer>
