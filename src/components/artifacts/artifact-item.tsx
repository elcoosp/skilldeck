...
export function ArtifactItem({ artifact, compact = false, onPinChange }: ArtifactItemProps) {
  ...
  const [pinMenuPos, setPinMenuPos] = useState<{ top: number; left: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        pinMenuRef.current && !pinMenuRef.current.contains(e.target as Node) &&
        pinButtonRef.current && !pinButtonRef.current.contains(e.target as Node)
      ) {
            ...
            {!compact && (
              <>
                ...
                <div className="relative shrink-0">
                  ...
                  {showPinMenu && pinMenuPos && createPortal(
                    <div
                      ref={pinMenuRef}
                    ...
                    >
                      <button
                        ...
                        onClick={() => { handlePinBranch(); setShowPinMenu(false); setPinMenuPos(null); }}
                      >
                        <span>Pin to Branch</span>
                        {isBranchPinned && <span className="text-primary font-semibold ml-2">✓</span>}
                      </button>
                      <button
                        className="w-full text-left px-2 py-1.5 hover:bg-muted/50 flex items-center justify-between transition-colors whitespace-nowrap"
                        onClick={() => { handlePinGlobal(); setShowPinMenu(false); setPinMenuPos(null); }}
                      >
                        <span>Pin Globally</span>
                        {isGlobalPinned && <span className="text-primary font-semibold ml-2">✓</span>}
                      </button>
                    </div>,
                    document.body
                  )}
