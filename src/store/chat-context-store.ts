...
interface ChatContextState {
  ...
  updateSkillLintResults: (conversationId: string, skillId: string, warnings: LintWarning[]) => void
}
...
export const useChatContextStore = create<ChatContextState>((set) => ({
  ...
  addSkill: (conversationId, skill) =>
    set((state) => {
      ...
      if (current.some((item) => item.type === 'skill' && item.data.id === skill.id)) return state
      const { lintWarnings: _ignored, ...rest } = skill
      const newItem: AttachedSkill = { type: 'skill', data: { ...rest, lintWarnings: undefined } }
      return {
        items: {
          ...state.items,
  ...
  addFile: (conversationId, file) =>
    set((state) => {
      const current = state.items[conversationId] || []
      if (current.some((item) => item.type === 'file' && item.data.id === file.id)) return state
      return {
        items: {
          ...state.items,
  ...
  addFolder: (conversationId, folder) =>
    set((state) => {
      const current = state.items[conversationId] || []
      if (current.some((item) => item.type === 'folder' && item.data.id === folder.id)) return state
      return {
        items: {
          ...state.items,
