...
async function splitFile(inputFile, output1, output2) {
  ...
  output1 = output1 || path.join(dirName, `${baseName}_part1${path.extname(inputFile)}`);
  output2 = output2 || path.join(dirName, `${baseName}_part2${path.extname(inputFile)}`);
  ...
  console.log(`Total size: ${totalSize} bytes, first part will be ${halfSize} bytes.`);
...
if (args.length < 1) {
  console.error('Usage: node split-file.js <input-file> [output-file1] [output-file2]');
  process.exit(1);
}
